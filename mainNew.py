...
from newLoader import ImageDataset
import numpy as np
import matplotlib.pyplot as plt
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Load data
root = "C:/Users/user/Desktop/EE443 Neural/Final Project/"
...
tr = 0.7
val = 0.1
dataset_tr   = ImageDataset(root_dir = root, trainPercent = tr, valPercent = val , flag = 'train', transform = transforms.Compose([transforms.RandomHorizontalFlip(), transforms.Resize((224,224),interpolation=Image.NEAREST),
                                                                                           transforms.ToTensor()]))
...
dataset_val  = ImageDataset(root_dir = root, trainPercent = tr, valPercent = val  , flag = 'validation',transform = transforms.Compose([transforms.Resize((224,224),interpolation=Image.NEAREST),
...
wordC = pd.read_hdf("eee443_project_dataset_train.h5", 'word_code')
...
#        captionOutStr = [wordDict[i.item()] for i in captions[0]]
#        print("\n", captionOutStr)
...
#    if epoch % save_every == 0:
    ...
    torch.save(decoder.state_dict(), "./models/decoder"+ time_stamp + ".pth")
    torch.save(encoder.state_dict(), "./models/encoder"+ time_stamp + ".pth")
